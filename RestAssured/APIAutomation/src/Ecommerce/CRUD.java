...
public class CRUD {
	...
	public static void main(String args[])
	{
		...
		given().log().all().spec(createOrder).body(co).when().post("/api/ecom/order/create-order")
		.then().log().all();
		...
		given().log().all().spec(deleteProduct).when().delete("/api/ecom/product/delete-product/{productID}").then().log().all().extract().response().asString();
